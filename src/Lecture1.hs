...
minmax x y z =
  let max_all = max x (max y z)
      min_all = min x (min y z)
   in max_all - min_all
...
strSum :: String -> Int
strSum str = lstSum (words str)
  where
    lstSum :: [String] -> Int
    lstSum lst
      | null lst = 0
      | otherwise = (read (head lst) :: Int) + lstSum (tail lst)
...
lowerAndGreater n list = show n ++ " is greater than " ++ show (countGreater n list) ++ " elements and lower than " ++ show (countLower n list) ++ " elements"
  where
    ...
    countLower n list
      | null list = 0
      | otherwise = if n < head list then 1 + countLower n (tail list) else countLower n (tail list)

    countGreater :: Int -> [Int] -> Int
    countGreater n list
      | null list = 0
      | otherwise = if n > head list then 1 + countGreater n (tail list) else countGreater n (tail list)
