...
minmax x y z = maxThree - minThree
  where 
...
subString start end = drop (max 0 start) . take (max 0 (end + 1))
...
-}
strSum :: String -> Int
strSum str = strLstSum strlst 
  where 
    strlst = words str
    strLstSum :: [String] -> Int
    strLstSum sl = case sl of
      [] -> 0
      (x: xs) -> (read x :: Int) + strLstSum xs
...
lowerAndGreater n list = show n ++ " is greater than " ++ show greaterCount ++ " elements and lower than " ++ show lowerCount ++  " elements"
  where 
    greaterCount = length (filter (< n) list)
    lowerCount = length (filter (> n) list)
